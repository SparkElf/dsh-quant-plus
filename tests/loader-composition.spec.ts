...
test('real Loader composition: 60 tools register and are model-visible', async () => {
...
test('real Loader composition: disposal removes the registered tools (HMR-safety)', async () => {
  const ctx = await boot()
  // 60 = 上游 59 + fork 新增的 quant_factor_evaluate_cs
  ...
  assert.equal(ctx.tools.schemas().length, 61)
  disposer()
  assert.equal(ctx.tools.schemas().length, 60)
})
